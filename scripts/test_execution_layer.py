...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test the R3.1 execution simulation layer.")
    ...
    parser.add_argument("--risk-policy", default=str(ROOT_DIR / "rules" / "risk_policy.yaml"))
    parser.add_argument("--portfolio-policy", default=str(ROOT_DIR / "rules" / "portfolio_policy.yaml"))
    parser.add_argument("--strategy-policy", default=str(ROOT_DIR / "rules" / "strategy_policy.yaml"))
    parser.add_argument("--execution-policy", default=str(ROOT_DIR / "rules" / "execution_policy.yaml"))
...
def main() -> None:
    ...
    risk_decision = build_exposure_decision(signal, policy=load_risk_policy(args.risk_policy))
    portfolio = build_portfolio_allocation(
        {"input": signal, "decision": risk_decision},
        policy=load_portfolio_policy(args.portfolio_policy),
    )
    strategy_route = build_strategy_route(portfolio, policy=load_strategy_policy(args.strategy_policy))
    execution = simulate_execution_layer(
        strategy_route,
        policy=load_execution_policy(args.execution_policy),
    )
    print(
        json.dumps(
            {
