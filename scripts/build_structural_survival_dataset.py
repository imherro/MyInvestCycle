...
from engine.structural_survival_builder import (
    build_macro_regime_sequence,
    build_structural_survival_dataset,
    save_structural_survival_dataset,
    structural_survival_summary,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the structural regime survival duration/event dataset.")
    ...
    parser.add_argument("--output", default=str(DATA_DIR / "structural_survival_dataset.json"))
    ...
    parser.add_argument("--max-event-rate", type=float, default=0.15)
    parser.add_argument("--min-bull-duration", type=int, default=10)
...
def main() -> None:
    ...
    if index_df.empty:
        raise RuntimeError("No index rows available for structural survival dataset build.")
    ...
    dataset = build_structural_survival_dataset(
        sequence.items,
    ...
    validate_structural_survival_dataset(
        ...
        min_bull_duration=args.min_bull_duration,
    )
    output_path = save_structural_survival_dataset(dataset, args.output)
    ...
    diagnostics = {
        ...
        "summary": structural_survival_summary(dataset),
        "regime_counts": {regime: sequence.regimes.count(regime) for regime in REGIMES},
        "macro_change_events": int(len(macro["macro_change_indices"])),
        "accepted_break_indices": int(len(macro["break_indices"])),
        "parameters": {
            "smooth_radius": int(args.smooth_radius),
            "persistence_days": int(args.persistence_days),
            "min_break_gap": int(args.min_break_gap),
            "max_event_rate": float(args.max_event_rate),
            "min_bull_duration": int(args.min_bull_duration),
        },
        "cache_only": bool(args.cache_only),
        "include_hsgt": bool(args.include_hsgt),
        "history_sample_size": int(args.history_sample_size),
        "leakage_guard": "features use only t and earlier rows; structural event uses filtered macro-regime changes as target construction",
    }
