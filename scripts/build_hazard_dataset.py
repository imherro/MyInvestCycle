...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the regime hazard X_t -> y_t dataset.")
    ...
    parser.add_argument("--output", default=str(DATA_DIR / "hazard_dataset.json"))
...
def main() -> None:
    ...
    if index_df.empty:
        raise RuntimeError("No index rows available for hazard dataset build.")
    ...
    dataset = build_hazard_dataset(sequence.items)
    validate_hazard_dataset(dataset)
    output_path = save_hazard_dataset(dataset, args.output)
    ...
    diagnostics = {
        ...
        "label_distribution": hazard_label_distribution(dataset),
        ...
        "leakage_guard": "features use only t and earlier rows; label uses shift(-1) regime comparison",
    }
