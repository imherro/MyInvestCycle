...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test the R2.2 strategy routing layer.")
...
def main() -> None:
    ...
    portfolio_policy = load_portfolio_policy(args.portfolio_policy)
    portfolio = build_portfolio_allocation(
        {"input": signal, "decision": risk_decision},
        policy=portfolio_policy,
    )
    strategy_policy = load_strategy_policy(args.strategy_policy)
    route = build_strategy_route(portfolio, policy=strategy_policy)
    print(
        json.dumps(
            {
                "input": signal,
